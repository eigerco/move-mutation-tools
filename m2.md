...
In a [previous blog post](https://www.eiger.co/blog/working-on-aptos-insights-into-mutation-testing-and-specification-assurance), we explained the concept of mutation testing. Here we'll expand on
...
As there are in total over 2000 surviving mutants, we can not cover them all. A few examples are given per operator.
...
+        {}
     ...
     public(package) fun get_account_from_pending_request(
...
This mutation operator replaces unary operators with other unary operators. For example, the ! operator can be replaced with the space.
...
This mutation operator replaces if/else expression with constant boolean values. For example, the if (cond) { ... } else { ... }
